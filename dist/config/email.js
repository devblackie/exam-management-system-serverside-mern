...
const logoPath = path_1.default.join(process.cwd(), "public", "acadedesk.png");
// ─── Helper — logo HTML ──────────────────────────────────────────────────────
function logoImg(width = 44, height = 44) {
    // We reference 'cid:applogo' which matches the attachment ID below
    return `<img src="cid:applogo" alt="${config_1.default.appName}" width="${width}" height="${height}" 
...
function buildAssignmentRows(ctx) {
    if (ctx.role === "lecturer") {
        const rows = [];
        if (ctx.schoolName)
            rows.push(detailRow("School", ctx.schoolName));
        if (ctx.departmentName)
            rows.push(detailRow("Department", ctx.departmentName));
        return rows.join("");
    }
...
function buildCoordSecNote(ctx) {
    if (ctx.role !== "coordinator" || !config_1.default.coodSec)
        return "";
    return `
    <div style="margin:24px 0;padding:14px 18px;border-radius:6px;
      ...
      <p style="margin:8px 0 0;text-align:center;font-family:'Courier New',monospace;font-size:15px;
        ...
        ${config_1.default.coodSec}
      </p>
    </div>`;
}
// ─── Main email sender ────────────────────────────────────────────────────────
const sendInviteEmail = async (ctx) => {
    const link = `${config_1.default.frontendUrl}/register/${ctx.token}`;
...
<head>
  ...
  <title>Invitation — ${config_1.default.appName}</title>
</head>
<body style="margin:0;padding:0;background:#f3f4f6;
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:40px 16px;">
          ...
          <tr>
            <td style="padding:36px 32px 28px;">
              ...
              <p style="margin:0 0 28px;font-size:14px;color:#6b7280;">
                ...
                on <strong style="color:#111827;">${config_1.default.appName}</strong>.
              </p>
          ...
          <tr>
            <td style="padding:20px 32px; border-top:1px solid #e5e7eb;">
               <p style="margin:0;font-size:11px;color:#9ca3af;text-align:center;">
                Sent by ${config_1.default.appName} · Authorized use only · Audit log active
              </p>
            </td>
          </tr>
        </table>
    ...
    await transporter.sendMail({
        ...
        to: ctx.to,
        subject: `Invitation — to join ${ctx.universityName} on ${config_1.default.appName}`,
        html,
