...
const getTransporter = () => nodemailer_1.default.createTransport({
    host: config_1.default.emailHot || "smtp.gmail.com",
    port: Number(config_1.default.emailPot) || 587,
    secure: false,
...
const sendOTPEmail = async (email, name, otp, purpose) => {
    const transporter = getTransporter();
    const subject = purpose === "login" ? "Your login verification code" : "Verify your registration";
    const fromName = config_1.default.appName || "EMS Academic System";
    const roleLabel = purpose === "login" ? "Login Verification" : "Registration Verification";
    await transporter.sendMail({
        from: `"${fromName}" <${config_1.default.emailUser}>`,
        to: email,
        subject,
