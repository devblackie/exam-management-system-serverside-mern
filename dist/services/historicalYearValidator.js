...
exports.validateHistoricalYears = validateHistoricalYears;
...
async function validateHistoricalYears(studentId, programId, yearsToCheck) {
    ...
    const pendingCFCodes = new Set((student.carryForwardUnits ?? [])
    ...
    for (let y = 1; y <= yearsToCheck; y++) {
        ...
        const pus = (await ProgramUnit_1.default.find({
            program: programId,
            requiredYear: y,
        })
            .populate("unit")
        ...
        for (const pu of pus) {
            ...
            const passingGrade = await FinalGrade_1.default.findOne({
                student: studentId,
                programUnit: puId,
                status: "PASS",
            }).lean();
            ...
            const suppGrade = await FinalGrade_1.default.findOne({
                student: studentId,
                programUnit: puId,
            ...
            const anyGrade = await FinalGrade_1.default.findOne({
                student: studentId,
                programUnit: puId,
            }).lean();
            if (!anyGrade) {
                ...
                const wasInYear = (student.academicHistory ?? []).some((h) => h.yearOfStudy === y);
                if (wasInYear) {
                    blockingUnits.push({
                        yearOfStudy: y,
                        unitCode,
...
// ── Helpers ───────────────────────────────────────────────────────────────────
async function getAttemptCount(studentId, puId) {
    return FinalGrade_1.default.countDocuments({ student: studentId, programUnit: puId });
}
function buildBlockReason(units) {
    const byYear = new Map();
    for (const u of units) {
        if (!byYear.has(u.yearOfStudy))
            byYear.set(u.yearOfStudy, []);
    ...
    return (`ENG.15(b) VIOLATION — Cannot enter final year. ` +
