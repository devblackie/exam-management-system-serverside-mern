...
// ── POST /programs ────────────────────────────────────────────────────────
router.post("/", auth_1.requireAuth, (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { name, code, description, durationYears, degreeType, schoolCode, departmentCode, } = req.body;
    // ── Validation ──────────────────────────────────────────────────────
    if (!name || !code) {
        throw { statusCode: 400, message: "Name and code are required" };
    }
    if (!schoolCode || !departmentCode) {
        throw { statusCode: 400, message: "School and department are required" };
    }
    const normalizedName = (0, programNormalizer_1.normalizeProgramName)(name.trim());
    const upperCode = code.trim().toUpperCase();
    // ── Check duplicate code (institution-wide) ─────────────────────────
    const existingByCode = await Program_1.default.findOne({
        code: upperCode,
        institution: req.user.institution,
    });
    if (existingByCode) {
    ...
    const existingByName = await Program_1.default.findOne({
        cleanName: normalizedName,
        institution: req.user.institution,
        schoolCode,
        departmentCode,
    ...
    (0, cache_1.invalidateCache)(`programs:${req.user.institution}`);
    res.status(201).json(program);
}));
// ── GET /programs ─────────────────────────────────────────────────────────
// router.get(
...
router.get("/", auth_1.requireAuth, (0, auth_1.requireRole)("admin", "coordinator"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const allowedIds = await (0, auth_1.getScopedProgramIds)(req);
    const institutionId = req.user.institution?.toString() ?? "";
    ...
    const programs = await (0, cache_1.cached)(cacheKey, // ← CHANGED — now per-user, not per-institution
    async () => {
        const all = await Program_1.default.find({
            institution: req.user.institution,
            _id: { $in: allowedIds },
            isActive: true,
        })
...
// ── PUT /programs/:id ─────────────────────────────────────────────────────
router.put("/:id", auth_1.requireAuth, (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const updateData = req.body;
    // ── Find existing ───────────────────────────────────────────────────
    const before = await Program_1.default.findOne({
        _id: id,
        institution: req.user.institution,
    }).lean();
    if (!before) {
    ...
    if (updateData.name && updateData.name !== before.name) {
        const normalizedName = (0, programNormalizer_1.normalizeProgramName)(updateData.name.trim());
        const nameConflict = await Program_1.default.findOne({
            _id: { $ne: id },
            cleanName: normalizedName,
            institution: req.user.institution,
            schoolCode: updateData.schoolCode || before.schoolCode,
    ...
    if (updateData.code && updateData.code.toUpperCase() !== before.code) {
        const codeConflict = await Program_1.default.findOne({
            _id: { $ne: id },
            code: updateData.code.toUpperCase(),
            institution: req.user.institution,
        });
    ...
    // ── Update ──────────────────────────────────────────────────────────
    const program = await Program_1.default.findOneAndUpdate({ _id: id, institution: req.user.institution }, { $set: updateData }, { new: true, runValidators: true });
