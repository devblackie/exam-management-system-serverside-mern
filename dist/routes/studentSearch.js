...
const multiTenant_1 = require("../lib/multiTenant");
const academicLeave_1 = require("../services/academicLeave");
const weightingRegistry_1 = require("../utils/weightingRegistry");
const studentStatusResolver_1 = require("../utils/studentStatusResolver");
...
async function attemptCount(student, programUnitId) {
    const reg = (student.unitAttemptRegistry || []).find((r) => r.unitId?.toString() === programUnitId);
    if (reg?.attempts?.length)
        return reg.attempts.length;
    return FinalGrade_1.default.countDocuments({ student: student._id, programUnit: programUnitId });
}
...
async function enrich(student, codes, yearPUs, extra) {
    const results = [];
    for (const code of codes) {
        const pu = yearPUs.find((p) => p.unit?.code?.toUpperCase() === code);
...
router.get("/search", auth_1.requireAuth, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { q } = req.query;
    if (!q || typeof q !== "string" || q.trim().length < 3)
        return res.status(400).json({ error: "Enter at least 3 characters" });
    const students = await Student_1.default.find((0, multiTenant_1.scopeQuery)(req, { regNo: { $regex: `^${q.trim()}`, $options: "i" } }))
        .limit(10).select("regNo name program admissionYear").populate("program", "name");
    res.json(students);
}));
router.get("/record", auth_1.requireAuth, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    let { regNo, yearOfStudy } = req.query;
    if (!regNo || typeof regNo !== "string")
        return res.status(400).json({ error: "regNo is required" });
    const targetYearOfStudy = parseInt(yearOfStudy) || 1;
    regNo = decodeURIComponent(regNo);
    const [student, settings] = await Promise.all([
        Student_1.default.findOne({ regNo: { $regex: `^${regNo}$`, $options: "i" } }).populate("program"),
        InstitutionSettings_1.default.findOne().lean(),
    ]);
    if (!student)
    ...
    const [grades, detailedMarks, directMarks] = await Promise.all([
        FinalGrade_1.default.find({ student: student._id }).populate({ path: "programUnit", populate: { path: "unit", select: "code name" } }).populate("academicYear", "year").lean(),
        Mark_1.default.find({ student: student._id }).lean(),
        MarkDirect_1.default.find({ student: student._id }).populate({ path: "programUnit", populate: { path: "unit", select: "code name" } }).populate("academicYear", "year").lean(),
    ]);
    const marksMap = new Map();
    detailedMarks.forEach((m) => marksMap.set(m.programUnit.toString(), m));
    const gradedUnitIds = new Set(grades.map((g) => g.programUnit?._id.toString()));
    const processedGrades = grades
        .filter((g) => g.programUnit?.requiredYear === targetYearOfStudy)
        .map((g) => {
    ...
    directMarks.forEach((dm) => {
        ...
        if (pUnit?.requiredYear === targetYearOfStudy && !gradedUnitIds.has(pUnit._id.toString())) {
            ...
            processedGrades.push({
                ...
                status: (() => {
                    if (dm.attempt === "special")
                        return "SPECIAL";
                    if (dm.attempt === "supplementary")
                        return "SUPPLEMENTARY";
                    if (dm.attempt === "re-take")
                        return "RETAKE";
    ...
    if (LOCKED.includes(student.status)) {
        const resolved = (0, studentStatusResolver_1.resolveStudentStatus)(student);
        const vm = { on_leave: "info", deferred: "info", discontinued: "error", deregistered: "error", graduated: "success", graduand: "success" };
        return res.json({
            student: { _id: student._id, name: student.name, regNo: student.regNo, programId: student.program?._id, currentYear: student.currentYearOfStudy, programName: student.program?.name, status: student.status },
            grades: sortedGrades,
    ...
    const academicStatus = await (0, statusEngine_1.calculateStudentStatus)(student._id, student.program._id, "", targetYearOfStudy);
    res.json({ student: { _id: student._id, name: student.name, regNo: student.regNo, programId: student.program?._id, currentYear: student.currentYearOfStudy, programName: student.program?.name, status: student.status }, grades: sortedGrades, academicStatus });
}));
const formatChallenges = (analysis) => ({
    supplementary: analysis.failedList.filter((f) => f.attempt === "A/S").map((f) => f.displayName.split(":")[0]),
    retakes: analysis.failedList.filter((f) => ["A/RA", "A/RPU"].includes(f.attempt)).map((f) => f.displayName.split(":")[0]),
    stayouts: analysis.failedList.filter((f) => f.attempt === "A/SO").map((f) => f.displayName.split(":")[0]),
    specials: analysis.specialList.map((s) => s.displayName.split(":")[0]),
...
//   }),
// );
router.get("/journey", auth_1.requireAuth, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { regNo } = req.query;
    if (!regNo)
        return res.status(400).json({ error: "regNo is required" });
    const student = (await Student_1.default.findOne((0, multiTenant_1.scopeQuery)(req, { regNo: regNo }))
        .populate("admissionAcademicYear", "year")
        .populate("program")
        .lean());
    if (!student)
    ...
    const admissionYearString = (() => {
        ...
        const sorted = [...(student.academicHistory || [])].sort((a, b) => (a.yearOfStudy || 0) - (b.yearOfStudy || 0));
        const earliest = sorted[0]?.academicYear;
        if (earliest && typeof earliest === "string" && earliest.includes("/"))
            return earliest;
        return "N/A";
    })();
    const incYear = (yr, offset) => {
        if (!yr || !yr.includes("/"))
            return yr;
    ...
    const allPUs = (await ProgramUnit_1.default.find({ program: program._id })
        .populate("unit")
        .lean());
    const pusByYear = new Map();
    ...
    try {
        ...
        const DC = (await Promise.resolve().then(() => __importStar(require("../models/DisciplinaryCase")))).default;
    ...
    for (const record of (student.academicHistory || [])) {
        if ((record.yearOfStudy || 1) > duration)
            continue;
        const weight = (0, weightingRegistry_1.getYearWeight)(program, entryType, record.yearOfStudy);
        ...
        const yearCFs = (student.carryForwardUnits || []).filter((u) => u.fromYear === record.yearOfStudy);
        const cfList = yearCFs.map((u) => ({
            code: u.unitCode,
            name: u.unitName,
        ...
        const yearDeferred = (student.deferredSuppUnits || []).filter((u) => u.fromYear === record.yearOfStudy);
        const deferredList = yearDeferred.map((u) => ({
            code: u.unitCode,
            name: u.unitName,
            reason: u.reason,
            status: u.status,
        ...
        if (!isGraduated) {
            try {
                const analysis = await (0, statusEngine_1.calculateStudentStatus)(student._id, student.program, resolvedYear, record.yearOfStudy, { forPromotion: false });
                totalUnits = analysis.summary.totalExpected || totalUnits;
                annualMean = parseFloat(analysis.weightedMean) || annualMean;
                if (!["ACADEMIC LEAVE", "GRADUATED", "DEFERMENT"].includes(analysis.status)) {
                    displayStatus = record.isRepeatYear
                        ? "REPEAT YEAR"
                ...
                const suppCodes = analysis.failedList
                    .filter((f) => f.attempt === "A/S")
                    .map((f) => codeOf(f.displayName));
                const retakeCodes = analysis.failedList
                    .filter((f) => ["A/RA", "A/RPU", "RP1C", "RP2C", "RP3C"].includes(f.attempt))
                    .map((f) => codeOf(f.displayName));
                const sosCodes = analysis.failedList
                    .filter((f) => f.attempt === "A/SO")
                    .map((f) => codeOf(f.displayName));
                const specCodes = analysis.specialList.map((s) => codeOf(s.displayName));
                const specGrounds = Object.fromEntries(analysis.specialList.map((s) => [
                ...
                challenges.specials = await enrich(student, specCodes, yearPUs, specGrounds);
        ...
        const yearQualifier = (() => {
            if (cfList.length > 0)
                return yearCFs[0]?.qualifier || "";
            if (record.isRepeatYear) {
                const repeatCount = (student.academicHistory || []).filter((h) => h.isRepeatYear && h.yearOfStudy <= record.yearOfStudy).length;
    ...
    const isLocked = LOCKED.includes(student.status);
    const currentInHistory = (student.academicHistory || []).some((h) => h.yearOfStudy === student.currentYearOfStudy);
    const withinDuration = (student.currentYearOfStudy || 1) <= duration;
    if (!isLocked && !currentInHistory && withinDuration) {
        const currentYearDoc = (await AcademicYear_1.default.findOne({
            institution: student.institution,
            isCurrent: true,
        }).lean());
        const weight = (0, weightingRegistry_1.getYearWeight)(program, entryType, student.currentYearOfStudy);
        const yearPUs = pusByYear.get(student.currentYearOfStudy) ?? [];
        const yearCFs = (student.carryForwardUnits || []).filter((u) => u.fromYear === student.currentYearOfStudy);
        const cfList = yearCFs.map((u) => ({
            code: u.unitCode,
            name: u.unitName,
            attempt: u.attemptNumber ?? 3,
        ...
        const yearDeferred = (student.deferredSuppUnits || []).filter((u) => u.fromYear === student.currentYearOfStudy);
        const deferredList = yearDeferred.map((u) => ({
            code: u.unitCode,
            name: u.unitName,
            reason: u.reason,
        ...
        try {
            const live = await (0, statusEngine_1.calculateStudentStatus)(student._id, student.program, "CURRENT", student.currentYearOfStudy);
            ...
            const sosCodes = live.failedList
                .filter((f) => f.attempt === "A/SO")
                .map((f) => codeOf(f.displayName));
            const specCodes = live.specialList.map((s) => codeOf(s.displayName));
            const specGrounds = Object.fromEntries(live.specialList.map((s) => [
                codeOf(s.displayName),
                s.grounds || "",
            ]));
            const incCodes = [...live.incompleteList, ...live.missingList].map(codeOf);
            const riskUnits = [];
            for (const pu of yearPUs) {
    ...
    // ═════════════════════════════════════════════════════════════════════
    for (const ev of (student.statusEvents || [])) {
    ...
    // ═════════════════════════════════════════════════════════════════════
    for (const entry of (student.deferredSuppUnits || [])) {
        journey.push({
            type: "DEFERRED_SUPP",
            academicYear: entry.fromAcademicYear || "",
            yearOfStudy: entry.fromYear || 0,
    ...
    // ═════════════════════════════════════════════════════════════════════
    const cfByYear = new Map();
    for (const u of (student.carryForwardUnits || [])) {
        if (!cfByYear.has(u.fromYear))
            cfByYear.set(u.fromYear, []);
        cfByYear.get(u.fromYear).push(u);
    }
    for (const [fromYear, units] of cfByYear) {
        const yearRecord = (student.academicHistory || []).find((h) => h.yearOfStudy === fromYear);
        const yearStr = yearRecord?.academicYear || incYear(admissionYearString, fromYear - 1);
        const qualifier = units[0]?.qualifier || "RP1C";
        journey.push({
            type: "CARRY_FORWARD",
            academicYear: yearStr,
            yearOfStudy: fromYear,
            cfUnits: units.map((u) => u.unitCode),
            qualifier,
            reason: `ENG.14: ${units.length} unit(s) carried forward to Year ${fromYear + 1}: ${units.map((u) => u.unitCode).join(", ")}`,
    ...
    for (const dc of disciplinaryCases) {
        journey.push({
            ...
            hearingDate: dc.hearingDate
                ...
                : undefined,
            reason: (dc.description || "").substring(0, 120) +
                ((dc.description || "").length > 120 ? "…" : ""),
            toStatus: dc.outcome === "SENT_HOME" ? "disciplinary_suspension" : dc.outcome?.toLowerCase(),
    ...
    // ═════════════════════════════════════════════════════════════════════
    if (isGraduated) {
        const lastHistory = [...(student.academicHistory || [])].sort((a, b) => (b.yearOfStudy || 0) - (a.yearOfStudy || 0))[0];
        journey.push({
            type: "GRADUATION",
    ...
    if (isGraduated) {
        ...
        if (stored > 0) {
            ...
            try {
                const { calculateGraduationStatus } = await Promise.resolve().then(() => __importStar(require("../services/graduationEngine")));
                const result = await calculateGraduationStatus(student._id.toString());
                projectedMean = result.weightedAggregateAverage;
            }
            catch {
                projectedMean = 0;
    ...
    else if (projectedMean >= 40)
        classification = "PASS";
    else if (projectedMean > 0)
        classification = "BELOW PASS MARK";
    return res.json({
        admissionYear: admissionYearString,
        intake: student.intake || "SEPT",
        currentStatus: student.status.toUpperCase(),
...
router.post("/approve-special", auth_1.requireAuth, (0, auth_1.requireRole)("coordinator"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    let mark = await Mark_1.default.findByIdAndUpdate(markId, upd, { new: true });
    if (!mark)
        mark = await MarkDirect_1.default.findByIdAndUpdate(markId, upd, { new: true });
    if (!mark)
        return res.status(404).json({ error: "Mark record not found" });
    try {
        const gr = await (0, gradeCalculator_1.computeFinalGrade)({ markId: mark._id, coordinatorReq: req });
...
router.post("/leave", auth_1.requireAuth, (0, auth_1.requireRole)("coordinator"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { studentId, startDate, endDate, reason, leaveType } = req.body;
    const result = await (0, academicLeave_1.grantAcademicLeave)(studentId, new Date(startDate), new Date(endDate), reason, leaveType);
    res.json({ success: true, student: result });
}));
router.post("/defer", auth_1.requireAuth, (0, auth_1.requireRole)("coordinator"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { studentId, years } = req.body;
    const result = await (0, academicLeave_1.deferAdmission)(studentId, parseInt(years));
    res.json({ success: true, student: result });
}));
router.post("/revert-active", auth_1.requireAuth, (0, auth_1.requireRole)("coordinator"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { studentId } = req.body;
    const result = await (0, academicLeave_1.revertStatusToActive)(studentId);
    res.json({ success: true, student: result });
}));
router.post("/readmit", auth_1.requireAuth, (0, auth_1.requireRole)("coordinator"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { studentId, remarks } = req.body;
    if (!studentId)
        return res.status(400).json({ error: "Student ID required" });
    const result = await (0, academicLeave_1.readmitStudent)(studentId, remarks || "Approved by Senate/Department");
...
router.get("/raw-marks", auth_1.requireAuth, (0, auth_1.requireRole)("coordinator"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { regNo, yearOfStudy } = req.query;
    if (!regNo)
        return res.status(400).json({ error: "regNo required" });
    const student = await Student_1.default.findOne({ regNo: { $regex: `^${regNo}$`, $options: "i" } });
    if (!student)
        return res.status(404).json({ error: "Student not found" });
    const [dm, dmt] = await Promise.all([
        Mark_1.default.find({ student: student._id }).populate({ path: "programUnit", match: yearOfStudy ? { requiredYear: parseInt(yearOfStudy) } : {}, populate: { path: "unit", select: "code name" } }).populate("academicYear", "year").lean(),
        MarkDirect_1.default.find({ student: student._id }).populate({ path: "programUnit", match: yearOfStudy ? { requiredYear: parseInt(yearOfStudy) } : {}, populate: { path: "unit", select: "code name" } }).populate("academicYear", "year").lean(),
    ]);
    res.json([...dm.filter((m) => m.programUnit).map((m) => ({ ...m, entryMode: "detailed" })), ...dmt.filter((m) => m.programUnit).map((m) => ({ ...m, entryMode: "direct" }))]);
}));
router.post("/raw-marks", auth_1.requireAuth, (0, auth_1.requireRole)("coordinator"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const [student, unitDoc, yearDoc] = await Promise.all([Student_1.default.findOne({ regNo: { $regex: `^${regNo}$`, $options: "i" } }), Unit_1.default.findOne({ code: unitCode.toUpperCase().trim() }), AcademicYear_1.default.findOne({ year: academicYear })]);
    if (!student || !unitDoc || !yearDoc)
        return res.status(404).json({ error: "Required metadata not found" });
    const programUnit = await ProgramUnit_1.default.findOne({ program: student.program, unit: unitDoc._id });
    if (!programUnit)
        return res.status(400).json({ error: `Unit ${unitCode} not linked to student's program.` });
    ...
    if (isDirectEntry) {
        ...
        const u = await MarkDirect_1.default.findOneAndUpdate({ student: student._id, programUnit: programUnit._id, academicYear: yearDoc._id }, upd, { upsert: true, new: true, setDefaultsOnInsert: true });
    ...
    const u = await Mark_1.default.findOneAndUpdate({ student: student._id, programUnit: programUnit._id, academicYear: yearDoc._id }, upd, { upsert: true, new: true, setDefaultsOnInsert: true });
    const gr = await (0, gradeCalculator_1.computeFinalGrade)({ markId: u._id, coordinatorReq: req });
...
// (ENG.13b / ENG.18c)
router.post("/defer-supp", auth_1.requireAuth, (0, auth_1.requireRole)("coordinator"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const pUnits = await ProgramUnit_1.default.find({
        _id: { $in: programUnitIds },
        program: student.program,
    }).populate("unit").lean();
    console.log("[defer-supp] programUnitIds requested:", programUnitIds);
    console.log("[defer-supp] pUnits found in DB       :", pUnits.map((p) => ({
        _id: p._id.toString(),
        code: p.unit?.code,
        program: p.program?.toString(),
    })));
    if (pUnits.length !== programUnitIds.length) {
        console.error(`[defer-supp] 400: ID count mismatch — requested ${programUnitIds.length}, found ${pUnits.length}`, "\nrequested:", programUnitIds, "\nfound    :", pUnits.map((p) => p._id.toString()), "\nstudent.program:", student.program?.toString());
    ...
    const existingDeferred = student.deferredSuppUnits || [];
    const alreadyDeferredIds = new Set(existingDeferred
        .filter((u) => u.status === "pending")
        .map((u) => u.programUnitId));
    const newUnitIds = programUnitIds.filter((id) => !alreadyDeferredIds.has(id));
    ...
    const entries = pUnits
        .filter((pu) => newUnitIds.includes(pu._id.toString()))
        .map((pu) => ({
    ...
    console.log("[defer-supp] writing entries:", entries.map((e) => ({ code: e.unitCode, reason: e.reason })));
    await Student_1.default.findByIdAndUpdate(studentId, {
        $push: {
            ...
            statusEvents: {
                ...
                date: new Date(),
                academicYear,
                reason: `ENG.13b/ENG.18c deferral — ${reason.replace("_", " ")}: ${entries.map((e) => e.unitCode).join(", ")}`,
            },
        },
    });
    await (0, auditLogger_1.logAudit)(req, {
        ...
        details: { units: entries.map((e) => e.unitCode), reason, academicYear },
    });
    console.log(`[defer-supp] ✓ deferred ${entries.length} unit(s) for student ${student.regNo}`);
    return res.json({
        success: true,
...
router.delete("/defer-supp/:studentId/:programUnitId", auth_1.requireAuth, (0, auth_1.requireRole)("coordinator"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
...
router.get("/deferred-units/:studentId", auth_1.requireAuth, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const student = await Student_1.default.findById(req.params.studentId)
        .select("deferredSuppUnits")
        .lean();
    if (!student)
        return res.status(404).json({ error: "Student not found" });
    const pending = (student.deferredSuppUnits || []).filter((u) => u.status === "pending");
