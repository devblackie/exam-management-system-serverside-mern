...
// //   })
// // );
// // serverside/src/routes/units.ts — REPLACE the POST handler
// router.post(
//   "/",
//   requireAuth,
//   requireRole("admin", "coordinator"),
...
//     const { code, name } = req.body;
//     const institutionId = req.user.institution;
//     const userDeptCode = (req.user as any).departmentCode;
//     const userSchoolCode = (req.user as any).schoolCode;
//     const isInstitutionWide = (req.user as any).institutionWide;
//     if (!code || !name) {
//       return res.status(400).json({ message: "Unit code and name are required." });
//     }
//     // ── Determine the department for this unit ──────────────────────────
//     // If the user is scoped to a department, that department is used.
//     // If the user is institution-wide (admin), departmentCode must be sent in the body.
//     let departmentCode: string;
//     let schoolCode: string;
//     if (userDeptCode) {
//       // Department coordinator — unit goes to their department
//       departmentCode = userDeptCode;
//       schoolCode = userSchoolCode;
//     } else if (req.body.departmentCode) {
//       // Admin creating unit for a specific department
//       departmentCode = req.body.departmentCode.toUpperCase();
//       schoolCode = req.body.schoolCode?.toUpperCase() || "";
//     } else {
//       return res.status(400).json({
//         message: "departmentCode is required when creating as admin. Coordinators auto-assign to their department.",
//       });
//     }
//     const unitCode = code.toUpperCase();
//     // ── Check duplicate within the department ───────────────────────────
//     const duplicate = await Unit.findOne({
//       code: unitCode,
//       departmentCode,
//       institution: institutionId,
...
//     if (duplicate) {
//       return res.status(400).json({
//         message: `Unit '${unitCode}' already exists in department '${departmentCode}'.`,
//       });
//     }
//     const newUnit = new Unit({
//       code: unitCode,
...
//     });
//     await newUnit.save();
//     invalidateCache(`units:${institutionId}:${departmentCode}`);
//     res.status(201).json({
...
// //   }
// // }));
// // serverside/src/routes/units.ts — REPLACE the GET handler
// router.get("/", requireAuth, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
//   const institutionId = req.user.institution;
...
//   const userDeptCode = (req.user as any).departmentCode;
//   const isInstitutionWide = (req.user as any).institutionWide;
//   // ── Admin or institution-wide: see ALL units ──────────────────────────
//   if (userRole === "admin" || isInstitutionWide) {
//     const units = await cached(`units:${institutionId}:all`, () =>
//       Unit.find({ institution: institutionId }).sort({ code: 1 }).lean()
//     );
//     return res.json(units);
//   }
//   // ── School coordinator (has schoolCode, maybe no departmentCode) ──────
//   // They see ALL units in their school, across all departments
//   if (userSchoolCode && !userDeptCode) {
...
//     return res.json(units);
//   }
//   // ── Department coordinator (has departmentCode) ───────────────────────
//   // They see ONLY units in their specific department
//   if (userDeptCode) {
//     const units = await Unit.find({
//       institution: institutionId,
//       departmentCode: userDeptCode,
...
//     return res.json(units);
//   }
//   // ── Fallback: no scope defined — return empty ─────────────────────────
//   return res.json([]);
// }));
...
// serverside/src/routes/units.ts
...
// ── POST /units — Create unit template ──────────────────────────────────
router.post("/", auth_1.requireAuth, (0, auth_1.requireRole)("admin", "coordinator"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const isInstitutionWide = req.user.institutionWide;
    if (!code || !name) {
        return res.status(400).json({ message: "Unit code and name are required." });
    }
    let departmentCode;
    ...
    else if (isInstitutionWide && req.body.departmentCode) {
        departmentCode = req.body.departmentCode.toUpperCase();
        schoolCode = req.body.schoolCode?.toUpperCase() || "";
    }
    else {
        return res.status(400).json({
            message: "departmentCode is required. Coordinators auto-assign to their department.",
        });
    }
    const unitCode = code.toUpperCase();
    const duplicate = await Unit_1.default.findOne({
        code: unitCode,
        departmentCode,
        institution: institutionId,
    });
    ...
    const newUnit = new Unit_1.default({
        code: unitCode,
        name: name.trim(),
        departmentCode,
        schoolCode,
    ...
    await newUnit.save();
    (0, cache_1.invalidateCache)(`units:${institutionId}:${departmentCode}`);
    res.status(201).json({
        message: "Unit created successfully",
        unit: newUnit,
    });
}));
// ── GET /units — List units scoped by user's department/school ──────────
router.get("/", auth_1.requireAuth, (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const isInstitutionWide = req.user.institutionWide;
    if (userRole === "admin" || isInstitutionWide) {
        const units = await (0, cache_1.cached)(`units:${institutionId}:all`, () => Unit_1.default.find({ institution: institutionId }).sort({ code: 1 }).lean());
        return res.json(units);
    }
    if (userSchoolCode && !userDeptCode) {
...
// ── PUT /units/:id — Update unit template ───────────────────────────────
router.put("/:id", auth_1.requireAuth, (0, auth_1.requireRole)("admin", "coordinator"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const existingUnit = await Unit_1.default.findById(unitId);
    if (!existingUnit) {
        return res.status(404).json({ message: "Unit template not found" });
    }
    if (existingUnit.institution.toString() !== institutionId.toString()) {
    ...
    // Coordinator can only edit units in their own department
    if (!isInstitutionWide && userDeptCode) {
        if (existingUnit.departmentCode !== userDeptCode) {
            return res.status(403).json({
                message: "You can only edit units in your own department.",
            });
        }
    }
    // Check linked status if modifying code or name
    if (code !== undefined || name !== undefined) {
        const linkedCount = await ProgramUnit_1.default.countDocuments({
            unit: unitId,
            institution: institutionId,
        });
        if (linkedCount > 0) {
            return res.status(400).json({
                message: `Cannot modify this unit. It is linked to ${linkedCount} program(s). Delink it first.`,
            });
        }
    }
    ...
    if (code !== undefined) {
        const duplicate = await Unit_1.default.findOne({
            code: updateData.code,
            departmentCode: existingUnit.departmentCode,
            institution: institutionId,
            _id: { $ne: unitId },
        });
        if (duplicate) {
            return res.status(400).json({
                message: `Unit code '${updateData.code}' already exists in this department.`,
            });
        }
    }
    const unit = await Unit_1.default.findOneAndUpdate({ _id: unitId, institution: institutionId }, updateData, { new: true, runValidators: true });
    if (!unit) {
        return res.status(404).json({ message: "Unit template not found" });
    }
    (0, cache_1.invalidateCache)(`units:${institutionId}:${existingUnit.departmentCode}`);
    res.json(unit);
}));
// ── DELETE /units/:id — Delete unit template ────────────────────────────
router.delete("/:id", auth_1.requireAuth, (0, auth_1.requireRole)("admin", "coordinator"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!isInstitutionWide && userDeptCode) {
        if (existingUnit.departmentCode !== userDeptCode) {
            return res.status(403).json({
                message: "You can only delete units in your own department.",
            });
        }
    }
    const linkedCount = await ProgramUnit_1.default.countDocuments({
    ...
    if (linkedCount > 0) {
        return res.status(400).json({
            message: `Cannot delete this unit. It is linked to ${linkedCount} program(s). Remove all curriculum links first.`,
        });
    }
    const unit = await Unit_1.default.findOneAndDelete({
        _id: unitId,
        institution: institutionId,
    });
    if (!unit) {
