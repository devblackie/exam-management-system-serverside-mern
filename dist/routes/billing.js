...
router.get("/summary", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const institutionId = req.user.institution.toString();
    const billing = await Billing_1.default.findOne({
        institution: institutionId,
    }).lean();
    if (!billing) {
        res.status(404).json({
            message: "No billing record found. Contact support to set up billing.",
        });
        return;
    }
    const [activeStudents, totalStudents] = await Promise.all([
        Student_1.default.countDocuments({ institution: institutionId, status: { $in: ["active", "repeat"] } }),
    ...
    res.json({
        ...
        planCatalogue: billingService_1.PLAN_CATALOGUE.map((p) => ({
            name: p.name,
...
router.get("/invoices", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const institutionId = req.user.institution.toString();
    ...
    let invoices = [...(billing.invoices ?? [])].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    if (status)
        invoices = invoices.filter((i) => i.status === status);
    const total = invoices.length;
    const pageNum = parseInt(page);
    const limitNum = parseInt(limit);
    const paginated = invoices.slice((pageNum - 1) * limitNum, pageNum * limitNum);
    res.json({
        total,
        page: pageNum,
        invoices: paginated,
...
router.get("/invoices/:id", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const institutionId = req.user.institution.toString();
    const { id } = req.params;
    const billing = await Billing_1.default.findOne({
        institution: institutionId,
...
router.post("/invoices/generate", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const institutionId = req.user.institution.toString();
    const invoice = await (0, billingService_1.generateInvoice)(institutionId);
    if (!invoice) {
        res
            .status(400)
            .json({
...
router.patch("/invoices/:id/pay", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const institutionId = req.user.institution.toString();
    const { id } = req.params;
    ...
    const result = await (0, billingService_1.recordPayment)({
        institutionId,
        invoiceId: id,
...
router.patch("/invoices/bulk-pay", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const institutionId = req.user.institution.toString();
...
router.patch("/invoices/:id/void", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const institutionId = req.user.institution.toString();
    const { id } = req.params;
    ...
    const result = await (0, billingService_1.voidInvoice)(institutionId, id, reason);
    if (!result.ok) {
        res.status(400).json({ message: result.message });
        return;
    }
...
router.get("/usage", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const institutionId = req.user.institution.toString();
    const billing = await Billing_1.default.findOne({ institution: institutionId })
    ...
    const latest = await (0, billingService_1.takeUsageSnapshot)(institutionId, billing.seatLimit);
    res.json({
        latest,
        history: (billing.usageHistory ?? [])
            .sort((a, b) => new Date(b.snapshotDate).getTime() -
            new Date(a.snapshotDate).getTime())
            .slice(0, 24), // Last 24 months
    });
}));
// ── PATCH /billing/plan ───────────────────────────────────────────────────────
router.patch("/plan", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const institutionId = req.user.institution.toString();
    const { newPlanName, reason, customSeatLimit, customBasePrice, customPerSeatRate, // ← new name for the extra seat rate
     } = req.body;
    if (!newPlanName?.trim()) {
        res.status(400).json({ message: "newPlanName is required." });
        return;
    }
    const result = await (0, billingService_1.changePlan)({
        institutionId,
        newPlanName,
...
router.patch("/contact", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const institutionId = req.user.institution.toString();
    ...
    await Billing_1.default.findOneAndUpdate({ institution: institutionId }, { $set: { billingContact: { name, email, phone, address } } });
    await (0, auditLogger_1.logAudit)(req, {
...
// ── PATCH /billing/cycle ──────────────────────────────────────────────────────
router.patch("/cycle", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const institutionId = req.user.institution.toString();
    ...
    await Billing_1.default.findOneAndUpdate({ institution: institutionId }, { $set: { billingCycle: cycle } });
    await (0, auditLogger_1.logAudit)(req, {
...
router.get("/plan-history", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const institutionId = req.user.institution.toString();
    const billing = await Billing_1.default.findOne({ institution: institutionId })
        .select("planHistory")
    ...
    res.json({ planHistory: billing.planHistory ?? [] });
}));
...
const verifyCronSecret = (req) => req.headers["x-cron-secret"] === process.env.CRON_SECRET;
router.post("/cron/invoices", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    if (!verifyCronSecret(req)) {
        res.status(403).json({ message: "Forbidden." });
        return;
    }
    const result = await (0, billingService_1.generateMonthlyInvoices)();
    res.json({ message: "Monthly invoice generation complete.", ...result });
}));
router.post("/cron/overdue", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    if (!verifyCronSecret(req)) {
        res.status(403).json({ message: "Forbidden." });
        return;
    }
    const count = await (0, billingService_1.markOverdueInvoices)();
...
router.get("/department-stats", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const institutionId = req.user.institution.toString();
    ...
    const deptStats = await Student_1.default.aggregate([
        {
            $match: {
        ...
        },
        {
            $lookup: {
                ...
                let: { deptCode: "$_id" },
                pipeline: [
                    { $match: { institution: new mongoose_1.default.Types.ObjectId(institutionId) } },
                    { $unwind: "$schools" },
...
router.get("/hierarchy", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const institutionId = req.user.institution.toString();
    const cacheKey = `billing:hierarchy:${institutionId}`;
    const result = await (0, cache_1.cached)(cacheKey, async () => {
        const billing = await Billing_1.default.findOne({ institution: institutionId })
        ...
        const settings = await InstitutionSettings_1.default.findOne({ institution: institutionId })
            .select("schools")
            .lean();
        const programStats = await Student_1.default.aggregate([
            {
                $match: {
                    institution: new mongoose_1.default.Types.ObjectId(institutionId),
                    status: { $in: ["active", "repeat"] },
        ...
        settings?.schools?.forEach((school) => {
            ...
            school.departments?.forEach((dept) => {
                deptNameMap.set(`${school.code.toUpperCase()}|${dept.code.toUpperCase()}`, dept.name);
            });
        });
        // Build hierarchy map
        ...
        const schools = Array.from(schoolMap.values())
            ...
            departments: Array.from(school.departments.values())
                ...
                overage: dept.deptSeatLimit
                    ...
                    : 0,
                programs: dept.programs.sort((a, b) => a.programName.localeCompare(b.programName)),
            }))
...
router.patch("/department-seats", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const institutionId = req.user.institution.toString();
    ...
    const existing = billing.departmentSeats.find(ds => ds.departmentCode === departmentCode.toUpperCase());
    if (existing) {
    ...
    await billing.save();
    (0, cache_1.invalidateCache)(`billing:hierarchy:${institutionId}`);
    await (0, auditLogger_1.logAudit)(req, {
...
router.post("/invoices/export", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const institutionId = req.user.institution.toString();
    const { status, invoiceIds } = req.body;
    const billing = await Billing_1.default.findOne({ institution: institutionId }).lean();
    if (!billing) {
        res.status(404).json({ message: "No billing record." });
        return;
    }
...
router.get("/email-logs", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const institutionId = req.user.institution.toString();
    const logs = await EmailLog_1.default.find({ institution: institutionId })
        .sort({ timestamp: -1 })
        .limit(10)
        .lean();
    res.json(logs);
...
router.post("/invoices/:id/resend-email", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const institutionId = req.user.institution.toString();
    ...
    await (0, billingService_1.sendInvoiceEmail)(billing, invoice);
