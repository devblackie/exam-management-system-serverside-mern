...
// serverside/src/routes/disciplinary.ts - WITH SCOPING
...
// Helper to check if coordinator can access a student
async function canAccessStudent(req, studentId) {
    if (req.user.role === "admin")
        return true;
    if (req.user.institutionWide)
        return true;
    const student = await Student_1.default.findById(studentId).populate("program", "departmentCode schoolCode").lean();
    if (!student)
        return false;
    const program = student.program;
    return program?.departmentCode === req.user.departmentCode &&
        program?.schoolCode === req.user.schoolCode;
}
// POST /disciplinary/raise
router.post("/raise", (0, auth_1.requireRole)("coordinator", "admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { studentId, grounds, description, hearingDate, academicYearId } = req.body;
    const institutionId = req.user.institution;
    if (!studentId || !grounds || !description) {
        throw {
            statusCode: 400,
    ...
    // SCOPING: Coordinator can only file cases for students in their department
    if (!await canAccessStudent(req, studentId)) {
        throw {
            statusCode: 403,
            message: "You can only file disciplinary cases for students in your department.",
        };
    }
    ...
    const priorStatus = student.status;
    const disciplinaryCase = await DisciplinaryCase_1.default.create({
        institution: institutionId,
        student: studentId,
        raisedBy: req.user._id,
        academicYear: yearId,
    ...
    const currentAcademicYear = await AcademicYear_1.default.findById(yearId).select("year").lean();
    student.status = "disciplinary_suspension";
    student.statusEvents.push({
        fromStatus: priorStatus,
        toStatus: "disciplinary_suspension",
        date: new Date(),
    ...
    await (0, auditLogger_1.logAudit)(req, {
        action: "disciplinary_case_raised",
        actor: req.user._id,
        details: { caseId: disciplinaryCase._id, grounds, studentId, priorStatus },
    });
    res.status(201).json({
        message: "Disciplinary case filed. Student has been suspended pending hearing.",
        caseId: disciplinaryCase._id,
    });
}));
// GET /disciplinary - SCOPED
router.get("/", (0, auth_1.requireRole)("coordinator", "admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { outcome, page = "1", limit = "20" } = req.query;
    const institutionId = req.user.institution;
    const filter = { institution: institutionId };
    if (outcome && typeof outcome === "string")
        filter.outcome = outcome;
    // SCOPING: Coordinators only see cases from their department
    if (req.user.role === "coordinator" && !req.user.institutionWide) {
        const scopedProgramIds = await (0, auth_1.getScopedProgramIds)(req);
        const studentsInDept = await Student_1.default.find({
            institution: institutionId,
            program: { $in: scopedProgramIds }
        }).distinct("_id");
        filter.student = { $in: studentsInDept };
    }
    const skip = (parseInt(page) - 1) * parseInt(limit);
    const total = await DisciplinaryCase_1.default.countDocuments(filter);
    const cases = await DisciplinaryCase_1.default.find(filter)
        .sort({ createdAt: -1 })
        .skip(skip)
        ...
        .populate("resolvedBy", "name email")
        .lean();
    res.json({ total, page: parseInt(page), cases });
}));
// GET /disciplinary/student/:studentId - SCOPED
router.get("/student/:studentId", (0, auth_1.requireRole)("coordinator", "admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { studentId } = req.params;
    const institutionId = req.user.institution;
    if (!mongoose_1.default.Types.ObjectId.isValid(studentId)) {
        throw { statusCode: 400, message: "Invalid student ID." };
    }
    // SCOPING: Coordinator can only view their own department's students
    if (!await canAccessStudent(req, studentId)) {
        throw {
            statusCode: 403,
            message: "You can only view disciplinary cases for students in your department.",
        };
    }
    const cases = await DisciplinaryCase_1.default.find({
        student: studentId,
        institution: institutionId,
    })
        .sort({ createdAt: -1 })
...
// GET /disciplinary/:caseId - SCOPED
router.get("/:caseId", (0, auth_1.requireRole)("coordinator", "admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { caseId } = req.params;
    const institutionId = req.user.institution;
    const disciplinaryCase = await DisciplinaryCase_1.default.findOne({
        _id: caseId,
        institution: institutionId,
    })
        .populate("student", "name regNo currentYearOfStudy status")
    ...
    // SCOPING: Coordinator can only view cases from their department
    if (req.user.role === "coordinator" && !req.user.institutionWide) {
        const studentId = disciplinaryCase.student?._id || disciplinaryCase.student;
        if (!await canAccessStudent(req, studentId.toString())) {
            throw {
                statusCode: 403,
                message: "You can only view disciplinary cases for students in your department.",
            };
...
// PATCH /disciplinary/:caseId/outcome - ADMIN ONLY (already restricted)
router.patch("/:caseId/outcome", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { caseId } = req.params;
    const { outcome, outcomeNotes, suspensionStart, suspensionEnd, hearingDate } = req.body;
    const institutionId = req.user.institution;
    const validOutcomes = ["WARNING", "SENT_HOME", "REINSTATED", "DISCONTINUED", "DISMISSED"];
    if (!validOutcomes.includes(outcome)) {
        throw { statusCode: 400, message: `outcome must be one of: ${validOutcomes.join(", ")}` };
    }
    const disciplinaryCase = await DisciplinaryCase_1.default.findOne({
        _id: caseId,
        institution: institutionId,
    ...
    if (!disciplinaryCase)
        throw { statusCode: 404, message: "Case not found." };
    if (disciplinaryCase.outcome !== "PENDING") {
        throw { statusCode: 409, message: "This case has already been resolved." };
    }
    const student = await Student_1.default.findById(disciplinaryCase.student);
    if (!student)
        throw { statusCode: 404, message: "Student record missing." };
    const currentAcademicYear = await AcademicYear_1.default.findById(disciplinaryCase.academicYear).select("year").lean();
    const yearStr = currentAcademicYear?.year ?? "Unknown";
    disciplinaryCase.outcome = outcome;
    disciplinaryCase.outcomeNotes = outcomeNotes;
    disciplinaryCase.resolvedBy = req.user._id;
    disciplinaryCase.resolvedAt = new Date();
    if (hearingDate)
        disciplinaryCase.hearingDate = new Date(hearingDate);
    if (suspensionStart)
        disciplinaryCase.suspensionStart = new Date(suspensionStart);
    if (suspensionEnd)
        disciplinaryCase.suspensionEnd = new Date(suspensionEnd);
    ...
    res.json({ message: `Disciplinary outcome recorded: ${outcome}`, studentStatus: newStudentStatus });
}));
// PATCH /disciplinary/:caseId/reinstate - ADMIN ONLY
router.patch("/:caseId/reinstate", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { caseId } = req.params;
    const { notes } = req.body;
    const institutionId = req.user.institution;
    const disciplinaryCase = await DisciplinaryCase_1.default.findOne({
    ...
    if (!disciplinaryCase)
        throw { statusCode: 404, message: "Case not found." };
    if (disciplinaryCase.outcome !== "SENT_HOME") {
        throw { statusCode: 409, message: "Only SENT_HOME cases can be reinstated." };
    }
    const student = await Student_1.default.findById(disciplinaryCase.student);
    if (!student)
        throw { statusCode: 404, message: "Student record missing." };
    const currentAcademicYear = await AcademicYear_1.default.findById(disciplinaryCase.academicYear).select("year").lean();
    disciplinaryCase.outcome = "REINSTATED";
    disciplinaryCase.outcomeNotes = (disciplinaryCase.outcomeNotes ?? "") + ` | REINSTATED: ${notes ?? ""}`;
    disciplinaryCase.resolvedAt = new Date();
    disciplinaryCase.resolvedBy = req.user._id;
    const restoredStatus = disciplinaryCase.priorStudentStatus;
    ...
    res.json({ message: "Student reinstated. Status restored.", restoredStatus });
}));
// PATCH /disciplinary/:caseId/appeal - ADMIN ONLY
router.patch("/:caseId/appeal", (0, auth_1.requireRole)("admin"), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { caseId } = req.params;
    const { appealDate, appealOutcome, appealNotes } = req.body;
    const institutionId = req.user.institution;
    if (!["UPHELD", "DISMISSED"].includes(appealOutcome)) {
        throw { statusCode: 400, message: "appealOutcome must be UPHELD or DISMISSED." };
    }
    const disciplinaryCase = await DisciplinaryCase_1.default.findOne({
        _id: caseId,
        institution: institutionId,
    ...
    if (!disciplinaryCase)
        throw { statusCode: 404, message: "Case not found." };
    if (disciplinaryCase.appealed) {
        throw { statusCode: 409, message: "An appeal has already been recorded for this case." };
    }
    disciplinaryCase.appealed = true;
    disciplinaryCase.appealDate = appealDate ? new Date(appealDate) : new Date();
    disciplinaryCase.appealOutcome = appealOutcome;
    disciplinaryCase.appealNotes = appealNotes;
    if (appealOutcome === "UPHELD") {
        const student = await Student_1.default.findById(disciplinaryCase.student);
        if (student) {
            student.status = disciplinaryCase.priorStudentStatus;
            student.qualifierSuffix = "";
            student.statusEvents.push({
    ...
    res.json({ message: `Appeal ${appealOutcome === "UPHELD" ? "upheld — student reinstated" : "dismissed — original outcome stands"}.` });
}));
