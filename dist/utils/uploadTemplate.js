...
exports.generateFullScoresheetTemplate = void 0;
...
const generateFullScoresheetTemplate = async (programId, unitId, yearOfStudy, semester, academicYearId, logoBuffer, examMode = "standard", unitType = "theory") => {
    // ── Metadata ──────────────────────────────────────────────────────────────
    ...
    const programUnit = await ProgramUnit_1.default.findOne({
        program: programId,
        unit: unitId,
    }).lean();
    ...
    const marksMap = programUnit
        ? await (0, scoresheetStudentList_1.getExistingMarksForStudents)(studentIds, programUnit._id)
        : new Map();
    ...
    const pinkColor = { type: "pattern", pattern: "solid", fgColor: { argb: "FFFFA6C9" } };
    ...
    dynamicMaxScores.forEach((val, i) => { if (val !== null)
        scoreRow.getCell(i + 1).value = val; });
    [14, 15, 16].forEach(rowNum => {
        ...
        for (let c = 1; c <= 24; c++) {
            ...
            if (rowNum === 14 && [5, 9, 15].includes(c))
                cell.fill = greyFill;
            ...
            if (unitType === "workshop" && c >= 5 && c <= 12)
                cell.fill = greyFill;
            if (unitType === "theory" && c === 13)
                cell.fill = greyFill;
        }
    });
    // ── Data rows ─────────────────────────────────────────────────────────────
    ...
    const applyValidation = (cellAddr, maxVal) => {
    ...
    for (let r = startRow; r <= endRow; r++) {
        ...
        for (let c = 1; c <= 24; c++) {
            ...
            if (ss && isSupp && (isCAInput || isPracInput)) {
                ...
                // Validation
                if (c === 5)
                    applyValidation(`E${r}`, settings.cat1Max);
                if (c === 6)
                    applyValidation(`F${r}`, settings.cat2Max);
                if (c === 7 && settings.cat3Max > 0)
                    applyValidation(`G${r}`, settings.cat3Max);
                if (c === 9 && settings.assignmentMax > 0)
                    applyValidation(`I${r}`, settings.assignmentMax);
                if (c === 13 && settings.practicalMax > 0)
                    applyValidation(`M${r}`, settings.practicalMax);
                if (c === 15)
                    applyValidation(`O${r}`, q1Max);
                if (c >= 16 && c <= 19)
                    applyValidation(`${sheet.getColumn(c).letter}${r}`, 20);
            }
            if (c >= 15 && c <= 19)
                cell.fill = purpleFill;
        }
    ...
    ["E", "F", "G", "H", "I", "J", "K", "O", "P", "Q", "R", "S"].forEach(col => (sheet.getColumn(col).width = 4.5));
    sheet.protect("1234", { selectLockedCells: true, selectUnlockedCells: true });
    const buf = await workbook.xlsx.writeBuffer();
    return Buffer.from(buf);
};
exports.generateFullScoresheetTemplate = generateFullScoresheetTemplate;
