...
exports.getYearWeight_ALL = exports.getYearWeight = exports.WEIGHTING_REGISTRY = void 0;
exports.WEIGHTING_REGISTRY = {
...
function resolveKey(prog) {
    ...
    if (prog?.schoolType) {
        ...
        if (st === "ENGINEERING" || st === "ENG") {
            ...
            if (exports.WEIGHTING_REGISTRY[key])
                return key;
        }
    }
    ...
    return exports.WEIGHTING_REGISTRY[genKey] ? genKey : "GEN_4";
}
// ─── Main export ───────────────────────────────────────────────────────────────
const getYearWeight = (prog, entry, year) => {
    const key = resolveKey(prog);
    const scheme = exports.WEIGHTING_REGISTRY[key] ?? exports.WEIGHTING_REGISTRY["GEN_4"];
...
exports.getYearWeight = getYearWeight;
// ─── Validate utility (call from a test route to verify all years) ─────────────
const getYearWeight_ALL = (prog, entry) => {
    const duration = prog?.durationYears || 5;
    const result = {};
    for (let y = 1; y <= duration; y++) {
        result[y] = (0, exports.getYearWeight)(prog, entry, y);
    }
    return result;
};
exports.getYearWeight_ALL = getYearWeight_ALL;
