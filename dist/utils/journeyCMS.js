...
exports.generateJourneyCMS = void 0;
...
const DOUBLE_BOTTOM = { ...THIN, bottom: { style: "double" } };
const F = {
    hdr: { type: "pattern", pattern: "solid", fgColor: { argb: "FFE0E0E0" } },
    subhdr: { type: "pattern", pattern: "solid", fgColor: { argb: "FFF2F2F2" } },
    pass: { type: "pattern", pattern: "solid", fgColor: { argb: "FFD9EAD3" } },
    passed: { type: "pattern", pattern: "solid", fgColor: { argb: "FFFFFFFF" } },
    supp: { type: "pattern", pattern: "solid", fgColor: { argb: "FFFFF2CC" } },
    fail: { type: "pattern", pattern: "solid", fgColor: { argb: "FFFFC7CE" } },
    special: { type: "pattern", pattern: "solid", fgColor: { argb: "FFE3D9F5" } },
    repeat: { type: "pattern", pattern: "solid", fgColor: { argb: "FFFCE4D6" } },
    gold: { type: "pattern", pattern: "solid", fgColor: { argb: "FFFFD700" } },
    blue: { type: "pattern", pattern: "solid", fgColor: { argb: "FFBDD7EE" } },
    amber: { type: "pattern", pattern: "solid", fgColor: { argb: "FFFCE4D6" } },
    none: { type: "pattern", pattern: "solid", fgColor: { argb: "FFFFFFFF" } },
    inc: { type: "pattern", pattern: "solid", fgColor: { argb: "FFFCE4D6" } },
    // CHANGE 2: New fill for disciplinary rows
    disciplinary: { type: "pattern", pattern: "solid", fgColor: { argb: "FFFFC7CE" } },
};
// ── Helpers ────────────────────────────────────────────────────────────────────
function addLogo(wb, sheet, logoBuffer, totalCols) {
...
function hdrRow(sheet, rowNum, text, totalCols, size = 10, bold = true, underline = false) {
    sheet.mergeCells(rowNum, 1, rowNum, totalCols);
    const cell = sheet.getCell(rowNum, 1);
    cell.value = text.toUpperCase();
    cell.alignment = { horizontal: "center", vertical: "middle" };
...
function hurdleSummary(yearsData, isDisciplinary) {
    if (isDisciplinary)
        return "DISCIPLINARY SUSPENSION";
    const events = [];
    for (const yr of yearsData) {
        const lbl = `Y${yr.yearOfStudy}`;
        if (yr.isRepeatYear)
            events.push(`${lbl}:REPEAT`);
        if (yr.suppUnits.length)
            events.push(`${lbl}:SUPP(${yr.suppUnits.join(",")})`);
        if (yr.passedSupp.length)
            events.push(`${lbl}:✓SUPP(${yr.passedSupp.join(",")})`);
        if (yr.cfUnits.length)
            events.push(`${lbl}:CF→Y${yr.yearOfStudy + 1}(${yr.cfUnits.join(",")})`);
        if (yr.passedCF.length)
            events.push(`${lbl}:✓CF(${yr.passedCF.join(",")})`);
        if (yr.specialUnits.length)
            events.push(`${lbl}:SPEC(${yr.specialUnits.join(",")})`);
    }
    return events.length ? events.join(" | ") : "CLEAN";
}
...
async function buildJourneyMarkMap(studentIds, puIds, passMark) {
    if (studentIds.length === 0 || puIds.length === 0)
        return new Map();
    // 3 parallel queries — one per collection
    const [allFinalGrades, allDirectMarks, allDetailedMarks] = await Promise.all([
        FinalGrade_1.default.find({
...
async function loadJourneys(programId, program, passMark, gradingScale) {
    const duration = program.durationYears || 5;
    const sortedScale = [...gradingScale].sort((a, b) => b.min - a.min);
    const gradeFor = (mark) => mark < passMark ? "E" : (sortedScale.find(s => mark >= s.min)?.grade ?? "E");
    ...
    const allPUs = await ProgramUnit_1.default.find({ program: programId }).populate("unit").lean();
    ...
    const markMapByYear = new Map();
    for (let y = 1; y <= duration; y++) {
        const pus = pusByYear.get(y) ?? [];
        const puIds = pus.map((pu) => pu._id.toString());
    ...
    for (const student of students) {
        ...
        const yearsData = [];
        for (let y = 1; y <= duration; y++) {
            const hist = history.find((h) => h.yearOfStudy === y);
            if (!hist)
                continue;
            const weight = (0, weightingRegistry_1.getYearWeight)(program, et, y);
            const annualMean = hist.annualMeanMark || 0;
            ...
            const studentYearMap = markMapByYear.get(y)?.get(sid) ?? new Map();
            for (const pu of pus) {
                ...
                if (isSpecial) {
                    ...
                    attemptLabel = "A/S";
                    if (status === "PASS")
                        passedSupp.push(code);
                    else
                        suppUnits.push(code);
                }
                else if (status !== "PASS" && !isSpecial) {
                    failedUnits.push(code);
                }
                const displayMark = isSpecial ? `${agreedMark}C` : agreedMark;
                unitMarks.push({
                    code,
                    name: uname,
                    mark: displayMark,
        ...
        if (storedWAA > 0) {
            ...
            const histWC = (student.academicHistory || []).reduce((sum, h) => sum + (h.weightedContribution || 0), 0);
            if (histWC > 0) {
                waa = histWC;
            }
            else {
                waa = yearsData.reduce((sum, yr) => sum + (yr.unitMarks.length > 0 ? yr.annualMean * yr.weight : 0), 0);
            }
        }
    ...
    records.sort((a, b) => {
        ...
        if (a.isDisciplinary && !b.isDisciplinary)
            return 1;
        if (!a.isDisciplinary && b.isDisciplinary)
            return -1;
        const ai = ORDER.indexOf(a.classification);
...
// ) {
function buildSummary(wb, records, program, logoBuffer, academicYear, meta) {
    const duration = program.durationYears || 5;
    ...
    sheet
        .getRow(lastData)
        .eachCell((c) => (c.border = { ...c.border, bottom: { style: "thick" } }));
    sheet.getColumn(1).width = 4;
    sheet.getColumn(2).width = 22;
    sheet.getColumn(3).width = 28;
    sheet.getColumn(4).width = 8;
    for (let y = 1; y <= duration; y++)
        sheet.getColumn(4 + y).width = 7;
    sheet.getColumn(5 + duration).width = 8;
    sheet.getColumn(6 + duration).width = 30;
    sheet.getColumn(7 + duration).width = 55;
    sheet.getColumn(8 + duration).width = 25;
...
// ) {
function buildYearSheet(wb, y, records, program, academicYear, passMark, logoBuffer, meta) {
    ...
    for (const rec of records) {
        ...
        if (rec.isDisciplinary)
            log.push("⚠ DISCIPLINARY SUSPENSION");
        if (yr.isRepeatYear)
            log.push("REPEAT YEAR");
        if (yr.suppUnits.length)
            log.push(`SUPP: ${yr.suppUnits.join(", ")}`);
        if (yr.passedSupp.length)
            log.push(`✓ SUPP CLEARED: ${yr.passedSupp.join(", ")}`);
        if (yr.cfUnits.length)
            log.push(`CF→Y${y + 1}: ${yr.cfUnits.join(", ")}`);
        if (yr.passedCF.length)
        ...
        if (rec.isDisciplinary)
            attempt = "SUSP.";
        else if (yr.isRepeatYear)
            attempt = "A/RA1";
        else if (rec.qualifierSuffix.includes("C"))
            attempt = rec.qualifierSuffix;
        ...
        codes.forEach((code) => {
            ...
            if (um && typeof um.mark === "number" && um.attemptNumber > 1) {
                const sup = ["", "¹", "²", "³", "⁴", "⁵"][Math.min(um.attemptNumber, 5)] ||
...
// ── LEGEND sheet ──────────────────────────────────────────────────────────────
function buildLegend(wb, logoBuffer) {
    const sheet = wb.addWorksheet("LEGEND");
    addLogo(wb, sheet, logoBuffer, 5);
    sheet.getColumn(1).width = 18;
    sheet.getColumn(2).width = 55;
    ...
    items.forEach(([code, meaning, fill], i) => {
        ...
        r.getCell(2).value = meaning;
        r.getCell(2).border = THIN;
        r.getCell(2).font = { name: FONT, size: 9 };
    });
    const ruleStart = 7 + items.length + 2;
    hdrRow(sheet, ruleStart, "KEY ENG RULES", 2, 9, true, true);
    [
        ...
        const r = sheet.getRow(ruleStart + 1 + i);
        r.height = 13;
        r.getCell(1).value = rule;
        r.getCell(1).font = { bold: true, name: FONT, size: 8 };
        r.getCell(2).value = desc;
        r.getCell(2).font = { name: FONT, size: 8 };
        [1, 2].forEach(c => (r.getCell(c).border = THIN));
    });
}
// ── MAIN EXPORT ───────────────────────────────────────────────────────────────
...
const generateJourneyCMS = async (data) => {
    const { programId, programName, academicYear, institutionId } = data;
    ...
    const [programDoc, settings, logoBuffer] = await Promise.all([
        ...
        (0, loadInstitutionSettings_1.loadInstitutionSettings)(institutionId),
        (0, loadLogoBuffer_1.loadLogoBuffer)(institutionId),
    ]);
    if (!programDoc)
...
exports.generateJourneyCMS = generateJourneyCMS;
