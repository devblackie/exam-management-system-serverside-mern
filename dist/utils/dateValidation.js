...
exports.requiredDateField = requiredDateField;
...
function validateDate(value) {
    ...
    if (isNaN(d.getTime()))
        return false;
    if (d.getTime() === 0)
        return false; // Unix epoch — invalid input
    if (d < MIN_DATE)
        return false; // Before year 2000
    if (d > MAX_DATE)
        return false; // Implausible far future
    return true;
}
/**
 ...
 */
function requiredDateField(fieldName) {
    return {
        type: Date,
        required: true,
        validate: {
 ...
 */
function optionalDateField(fieldName) {
    return {
        type: Date,
        validate: {
