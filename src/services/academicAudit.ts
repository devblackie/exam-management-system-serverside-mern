...
export const performAcademicAudit = async ( studentId: string, session?: mongoose.ClientSession ) => {
  ...
  const fatalUnitFailure = await FinalGrade.findOne({ student: studentId, attemptNumber: { $gte: 5 }, status: { $ne: "PASS" }})
    .populate({ path: "programUnit", populate: { path: "unit" } }).session(session || null);

  if (fatalUnitFailure) {
    const unitInfo = (fatalUnitFailure.programUnit as any)?.unit?.code || "Unknown Unit";
    await Student.findByIdAndUpdate( studentId,
      { $set: { status: "discontinued", remarks: `ENG 22.b.i Violation: Failed unit ${unitInfo} on attempt #${fatalUnitFailure.attemptNumber}.` }},
      { session },
    );
  ...
  if ( student.status === "active" && student.academicHistory?.some((h) => h.yearOfStudy === student.currentYearOfStudy && h.isRepeatYear )
  ) {
    // We call the status engine to get the current performance for the year
    const performance = await calculateStudentStatus( studentId.toString(), student.program.toString(), "N/A", student.currentYearOfStudy );

    if (performance.status === "REPEAT YEAR") {
      await Student.findByIdAndUpdate( studentId,
        { $set: { status: "discontinued", remarks: `ENG 22.b.ii Violation: Failed to clear Repeat Year requirements (Mean < 40% or > 50% units failed).`}},
        { session },
      );
