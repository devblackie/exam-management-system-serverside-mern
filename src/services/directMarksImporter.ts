...
//   if (!unitCode || !academicYearStr) {
//     throw new Error(
//       `Metadata missing. Unit code at F12: "${unitCode}", Academic year at C8: "${academicYearStr}".`,
//     );
//   }
...
//   // Pre-fetch ALL ProgramUnits for this unit (to avoid per-row DB calls)
...
//         if (!found) {
//           result.errors.push(
//             `Row ${rowNum} (${regNo}): Unit "${unitCode}" not linked to student's curriculum.`,
//           );
//           continue;
//         }
//         programUnit = found;
...
//       // ── Atomic upsert — no session needed ────────────────────────────────────
//       const saved = await MarkDirect.findOneAndUpdate(
//         {
//           student: (student as any)._id,
//           programUnit: (programUnit as any)._id,
...
//       // ── Compute final grade ───────────────────────────────────────────────────
//       // computeFinalGrade opens its own internal logic — no outer session needed.
//       try {
...
//       } catch (gradeErr: unknown) {
//         const msg =
//           gradeErr instanceof Error ? gradeErr.message : String(gradeErr);
//         console.warn(
