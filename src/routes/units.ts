...
router.post(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const { code, name } = req.body;
    const institutionId = req.user.institution;
    const userDeptCode = (req.user as any).departmentCode as string | undefined;
    const userSchoolCode = (req.user as any).schoolCode as string | undefined;
    const isInstitutionWide = (req.user as any).institutionWide as boolean | undefined;
...
router.put(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    invalidateCache(`units:${institutionId}:${existingUnit.departmentCode}`);
    res.json(unit);
  })
);
