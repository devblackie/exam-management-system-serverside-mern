// // serverside/src/utils/validateRegNo.ts — NEW FILE
// //
// // Validates a student registration number against the configured patterns
// // for their program's department.
// //
// // How it works:
// // 1. Load the program → find its school/department code
// // 2. Load InstitutionSettings → find the department's regNoPatterns
// // 3. Test the regNo against each pattern
// // 4. Return valid/invalid + which pattern matched
...
//   valid: boolean;
//   matchedExample?: string; // e.g. "E024-0001" — shown in error messages
...
//   yearDigits: number,
// ): RegExp {
//   // Pattern: PREFIX + YEAR(n digits) + SEPARATOR + SEQUENCE(1-6 digits)
//   // DeKUT Civil: "E" + "024" + "-" + "0001"  → /^E\d{3}-\d{1,6}$/
//   // JKUAT SMS:   "SMS" + "211" + "" + "XXXX"  → /^SMS\d{3}\d{1,6}$/
//   const sep = separator
//     ? separator.replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&")
//     : "";
//   return new RegExp(`^${prefix}\\d{${yearDigits}}${sep}\\d{1,6}$`);
// }
...
//   // Load program to get school/department code
//   const program = (await Program.findById(programId)
//     .select("schoolCode departmentCode")
//     .lean()) as any;
...
//   // Load institution settings (cached)
//   const settings = await cached(
//     `regno-patterns:${institutionId}:${program.departmentCode}`,
//     async () => {
//       const doc = await InstitutionSettings.findOne({
//         institution: institutionId,
//       }).lean();
//       if (!doc) return null;

//       for (const school of doc.schools) {
//         for (const dept of school.departments) {
//           if (dept.code === program.departmentCode) {
...
//   // If no patterns configured, skip validation
//   const doc = await InstitutionSettings.findOne({ institution: institutionId })
//     .select("enforceRegNoPattern")
//     .lean();

//   if (!doc?.enforceRegNoPattern || !settings || settings.length === 0) {
//     return { valid: true }; // validation not enforced — allow all
//   }

//   // Test against each configured pattern
//   for (const pattern of settings) {
//     if (pattern.manualRegex) {
//       try {
//         if (new RegExp(pattern.manualRegex).test(regNo)) {
//           return { valid: true, matchedExample: pattern.example };
//         }
//       } catch {
...
//   const examples = settings.map((p) => p.example).join(", ");
//   return {
//     valid: false,
//     reason: `Registration number "${regNo}" does not match the expected format for this department. Expected formats: ${examples}`,
//   };
// }
