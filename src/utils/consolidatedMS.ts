...
// ══════════════════════════════════════════════════════════════════════════════
// ROOT CAUSE OF CRASH:
//   TypeError: Cannot read properties of undefined (reading 'richText')
//   at SharedStringsXform.add  (exceljs internals)
//
// CAUSE: The function `buildAttemptNotation` was placed DIRECTLY into rowData[]:
//
//   const rowData: any[] = [
//     currentIndex + 1,
//     displayRegNo,
//     finalDisplayName,
//     buildAttemptNotation,   ← THE FUNCTION OBJECT ITSELF — not a call result
//   ];
//
// ExcelJS calls SharedStringsXform.add() on every cell value. It expects a
// string, number, boolean, Date, or null. When it receives a function object,
// it tries to access `.richText` on it → undefined → crash.
//
// FIX: call buildAttemptNotation(...) and place its string RETURN VALUE:
//
