...
// //   1. Student pool built by buildScoresheetStudentList() — single source of truth
// //      for who appears on which scoresheet and with what attempt label.
// //   2. displayRegNo (regNo + qualifierSuffix) written to Col B instead of raw regNo.
// //   3. SUPPLEMENTARY session: only SUPP + SPECIAL students (ENG.13 / ENG.18).
// //      STAYOUT and REPEAT YEAR students are excluded.
// //   4. ORDINARY session: primary + carry-forward (ENG.14) + stayout retakes (ENG.15h).
// //   5. Supp cap formula uses "A/S" attempt label (not the old "Supp" string).
// //   6. Special CA cell pre-populated and locked (ENG.18c).
// //   7. All attempt labels come from ScoresheetStudent.attemptLabel — no local logic.
...
// import { ATTEMPT_LABELS } from "./academicRules";
...
//   // buildScoresheetStudentList enforces ALL ENG rules:
//   //   ORDINARY  → active + repeat + CF students + stayout retakes
//   //   SUPP      → only SUPP-eligible (≤1/3 failed) + SPECIAL students
//   const eligibleStudents: ScoresheetStudent[] = await buildScoresheetStudentList({
//     programId,
//     programUnitId:  programUnitDoc?._id ?? unitId,
//     unitId,
//     yearOfStudy,
//     academicYearId,
//     session,
...
//   // ── Pre-existing marks map (for cell pre-population) ─────────────────────
...
//     bottom: { style: "thin" }, right: { style: "thin" },
//   };
//   const doubleBorder: Partial<ExcelJS.Borders> = { ...thinBorder, bottom: { style: "double" } };

//   const examTypeLabel = session === "SUPPLEMENTARY"
//     ? "SUPPLEMENTARY AND SPECIAL EXAMINATION"
//     : "EXAMINATION";
...
//   // ── Header rows (unchanged from original) ─────────────────────────────────
//   sheet.mergeCells("C6:G6");
//   sheet.getCell("C6").value = config.instName.toUpperCase();
//   sheet.getCell("C6").style = { ...centerBold, font: { ...centerBold.font, size: 12 } };
...
//       // ── Col B: displayRegNo carries the qualifier suffix ─────────────────
//       // e.g. "E024-01-1339/2016RP1" for a repeat student
//       row.getCell(1).value = idx + 1;
...
//       // ── Pre-populate CA for special students (ENG.18c) ───────────────────
//       // Special exams are marked out of 100% including CA.
//       // Lock the existing CA value so it cannot be changed.
//       if (ss.isSpecial && prevMark) {
//         const ca = prevMark.caTotal30 ?? 0;
//         if (ca > 0) row.getCell(5).value = ca;
//       }
//     }
...
//     // Internal total:
//     //   SUPP   → CA forced to 0 (ENG.13f — supp scored from exam only, max 40%)
//     //   SPECIAL → CA locked to pre-existing value
//     //   Normal  → CA + Exam
//     const caVal = ss?.isSupp ? "0" : `E${r}`;
//     row.getCell(7).value = {
//       formula: `IF(${isRowEmpty}, "", ROUND(${caVal} + F${r}, 0))`,
//     };

//     // Agreed mark — supp capped at passMark (ENG.13f)
//     // Attempt label for supp is now "A/S" (from ATTEMPT_LABELS.SUPPLEMENTARY)
//     const internal    = `G${r}`;
//     const external    = `H${r}`;
//     const effective   = `IF(${external}<>"", ${external}, ${internal})`;
//     const finalAgreed = ss?.isSupp
//       ? `MIN(${passMark}, ${effective})`
//       : effective;
...
//           cell.protection = { locked: true };
//           cell.value      = 0;
//         } else if (ss?.isSpecial) {
//           // CA pre-populated from previous mark, locked
//           cell.fill       = greyFill;
//           cell.protection = { locked: true };
//         } else {
//           // Editable CA input
...
//           cell.protection = { locked: false };
//           cell.dataValidation = {
//             type: "decimal", operator: "between", formulae: [0, 30],
...
//         cell.protection = { locked: false };
//         cell.dataValidation = {
//           type: "decimal", operator: "between", formulae: [0, 100], allowBlank: true,
//         };
//       } else if (c >= 7) {
//         cell.fill       = greyFill;
//         cell.protection = { locked: true };
...
//   // ── Column widths ─────────────────────────────────────────────────────────
//   sheet.getColumn(1).width = 4;
//   sheet.getColumn(2).width = 26;   // wider — qualifier suffix can be long
//   sheet.getColumn(3).width = 35;
//   sheet.getColumn(4).width = 8;    // wider — attempt labels like "A/SO", "RP1C"
//   sheet.getColumn(10).width = 6;
//   [5, 6, 7, 8, 9].forEach(c => (sheet.getColumn(c).width = 12));
...
import config from "../config/config";
import mongoose from "mongoose";
import { buildScoresheetStudentList, getExistingMarksForStudents, ScoresheetStudent } from "./scoresheetStudentList";
...
export const generateDirectScoresheetTemplate = async (
  ...
  for (let r = startRow; r <= endRow; r++) {
    ...
    if (ss) {
      ...
      row.getCell(3).font  = { name: fontName, size: 8 };
      if (ss.isSpecial && prevMark && (prevMark.caTotal30 ?? 0) > 0) {
        row.getCell(5).value = prevMark.caTotal30;
      }
    }
    ...
    for (let c = 1; c <= 10; c++) {
      ...
      if (c === 5) {
        if (ss?.isSupp)    { cell.fill = greyFill;   cell.protection = { locked: true }; cell.value = 0; }
        else if (ss?.isSpecial) { cell.fill = greyFill;   cell.protection = { locked: true }; }
        else               { cell.fill = pinkFill;   cell.protection = { locked: false };
          cell.dataValidation = { type: "decimal", operator: "between", formulae: [0, 30], allowBlank: true, showErrorMessage: true, errorTitle: "Invalid CA", error: "0–30" };
        }
      } else if (c === 6) {
