...
//   1. Student pool built by buildScoresheetStudentList() — same function used
//      by directTemplate.ts. One source of truth for all ENG rules.
//   2. displayRegNo written to Col B (qualifier suffix appended).
//   3. SUPPLEMENTARY: only SUPP-eligible (≤1/3 failed) + SPECIAL (ENG.13/18).
//      STAYOUT and REPEAT YEAR are excluded.
//   4. ORDINARY: primary + CF students (ENG.14) + stayout retakes (ENG.15h).
//   5. Attempt labels come from ScoresheetStudent.attemptLabel — no local logic.
//   6. Special CA columns pre-populated and locked (ENG.18c).
//   7. CA cap formula uses "A/S" label (not old "Supp" string).
//   8. All existing formula/validation/styling logic preserved exactly.
...
export const generateFullScoresheetTemplate = async (
  ...
  const headers = [
    ...
    "PRACTICAL", `CA TOTAL (${caWeightTotal}%)`,
    `Q1 (/${q1Max})`, "Q2", "Q3", "Q4", "Q5",
    `EXAM (${weights.exam}%)`,
  ...
  for (let r = startRow; r <= endRow; r++) {
    ...
    if (ss) {
      ...
      if (ss.isSpecial && prevMark) {
        row.getCell(5).value  = prevMark.cat1Raw    ?? 0;
        row.getCell(6).value  = prevMark.cat2Raw    ?? 0;
        row.getCell(7).value  = prevMark.cat3Raw    ?? 0;
        row.getCell(9).value  = prevMark.assgnt1Raw ?? 0;
        row.getCell(13).value = prevMark.practicalRaw ?? 0;
        // Lock these pre-populated cells
        for (let c = 5; c <= 13; c++) {
    ...
    for (let c = 1; c <= 24; c++) {
      ...
      if (ss && isSupp && (isCAInput || isPracInput)) {
        ...
        cell.fill       = greyFill;
        cell.protection = { locked: true };
      } else if (ss && isSpecial && (isCAInput || isPracInput)) {
