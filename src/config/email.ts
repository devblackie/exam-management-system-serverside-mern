...
export const sendInviteEmail = async ( to: string, token: string, name: string ) => {
  ...
  await transporter.sendMail({
    ...
    html: `
     ...
     <p>Welcome to <strong>${config.appName}</strong></p>
        <p>You have been invited to join the examination management system on <strong>${config.appName}</strong>.</p>
